...
const BoxSection: React.FC<BoxSectionProps> = ({ showExample, setShowExample, boxes, setBoxes }) => {
  ...
  // Permet de passer une box en pleine largeur ou de revenir à la largeur précédente
  function handleToggleFullWidth(idx: number): void {
    setBoxes(prev => prev.map((box, i) => {
      if (i !== idx) return box;
      if (box.fullWidth) {
        return { ...box, fullWidth: false, width: box.prevWidth };
      } else {
        return { ...box, fullWidth: true, prevWidth: box.width };
      }
    }));
  }
  ...
  function handleResizeBox(idx: number, width: number, height: number): void {
    setBoxes(prev => prev.map((box, i) => i === idx ? { ...box, width, height } : box));
  }
        ...
        {showExample ? (
          <div className="bg-amber-50 p-4 rounded-xl border border-amber-100 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {exampleBoxes.map((box, idx) => (
                <div key={idx} className={`border ${BORDER_COLORS[box.colorIdx]} rounded-lg p-4 bg-white shadow-sm`}>
          ...
          <div className="flex flex-wrap gap-4">
            {boxes.map((box, idx) => (
              <ResizableBox
                key={idx}
                width={box.fullWidth ? (box.width || 0) || 9999 : (box.width || 320)}
                height={box.height || 320}
                minWidth={180}
                minHeight={180}
                onResize={(width, height) => handleResizeBox(idx, width, height)}
                className={`border ${BORDER_COLORS[box.colorIdx % BORDER_COLORS.length]} rounded-lg p-4 ${COLORS[box.colorIdx % COLORS.length]} shadow-sm`}
                style={{ 
                  width: box.fullWidth ? '100%' : undefined, 
                  flex: box.fullWidth ? '1 1 100%' : undefined 
                }}
              >
                {/* Barre titre + actions */}
                <div className="flex items-center mb-3 relative">
                  ...
                  <div className="absolute right-2 top-1 flex gap-2 z-10">
                    ...
                    <button
                      type="button"
                      className="text-gray-400 hover:text-[#bd8c0f] focus:outline-none p-1"
                      title={box.fullWidth ? "Revenir à la largeur initiale" : "Passer en pleine largeur"}
                      onClick={() => handleToggleFullWidth(idx)}
                    >
                      {box.fullWidth ? '🗗' : '🗖'}
                    </button>
                  </div>
                ...
                {box.editing !== false ? (
                  <textarea
                    id={`box-content-${idx}`}
                    className="w-full p-4 border border-gray-200 rounded-xl min-h-[60px] text-[1rem] font-normal focus:outline-none focus:ring-2 focus:ring-amber-200 focus:border-[#bd8c0f] bg-white shadow-sm transition-all resize-none"
                    value={box.content}
                  ...
                  box.content && (
                    <div
                      className="min-h-[100px] p-3 rounded-lg text-gray-700 bg-white shadow-sm transition-all cursor-pointer"
